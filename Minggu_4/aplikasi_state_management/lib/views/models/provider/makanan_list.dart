...
import 'package:aplikasi_state_management/views/models/provider/detail_makanan.dart';
...
import 'package:aplikasi_state_management/views/models/provider/select_item.dart';
...
class MakananList extends StatefulWidget {
  ...
  @override
  _MakananListState createState() => _MakananListState(doneList);
}

class _MakananListState extends State<MakananList> {
  final List<MakananTradisional> doneList;
  final List<MakananTradisional> makananList = [
    MakananTradisional(
      nama: 'Mi Aceh',
      daerah: 'Aceh',
      gambar:
  ...
  _MakananListState(this.doneList);

  @override
  Widget build(BuildContext context) {
    return ListView.builder(itemBuilder: (context, index) {
      final MakananTradisional makanan = makananList[index];
      return InkWell(onTap: () {
        Navigator.push(
          context,
          MaterialPageRoute(
            builder: (context) => DetailMakanan(makananList: makanan),
          ),
        );
      }, child: Consumer<DoneProvider>(
        builder: (context, DoneProvider data, widget) {
          return SelectItem(
              makananList: makanan,
              isDone: data.doneProvider.contains(makanan),
              onCheckBoxClicked: (bool? value) {
                setState(() {
                  if (value != null) {
                    value
                        ? data.doneProvider.add(makanan)
                        : data.doneProvider.remove(makanan);
                  }
                });
              });
        },
